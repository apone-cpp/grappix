...
void RenderTarget::dashed_line(float x0, float y0, float x1, float y1,
                               uint32_t color)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, points.size() * 4, &points[0],
                 GL_DYNAMIC_DRAW);
...
void RenderTarget::draw_texture(GLint texture, float x, float y, float w,
                                float h, const float* uvs,
                                const Program& program, uint32_t color) const
{

    if (recBuf == -1) {
        static vector<float> p{-1, 1,  0, 0, 1, 1, 1, 0, -1, -1, 0, 1,
...
void RenderTarget::draw_textures(GLint texture, float* points, int count,
                                 float w, float h, const float* uvs,
                                 const Program& program) const
{
    ...
    glBufferSubData(GL_ARRAY_BUFFER, count * 8 * 4, coords.size() * 4,
                    &coords[0]);
    ...
    program.vertexAttribPointer("vertex", 2, GL_FLOAT, GL_FALSE, 0,
                                count * 8 * 4);
...
void RenderTarget::rectangle(float x, float y, float w, float h, uint32_t color,
...
void RenderTarget::text(const Font& font, const std::string& text, float x,
                        float y, uint32_t col, float scale) const
{
    font.render_text(*this, text, x, y, col, scale);
}

void RenderTarget::text(const std::string& text, float x, float y, uint32_t col,
                        float scale) const
{
