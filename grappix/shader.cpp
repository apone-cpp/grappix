...
GLuint createProgram(unsigned char* vertexSource, int vlen,
                     unsigned char* fragmentSource, int flen)
{
...
GLuint createProgram(const string& vertexSource, const string& fragmentSource)
{
    ...
    if (program) {
        ...
        if (linkStatus != GL_TRUE) {
            ...
            if (bufLength) {
                auto* buf = ALLOCA(char, bufLength);
...
const Program& get_program(program_name program)
{
    ...
    if (programs.size() == 0) {
        ...
        programs[FLAT_PROGRAM] =
            Program(_shader_plain_v_glsl, _shader_plain_v_glsl_size,
                    _shader_plain_f_glsl, _shader_plain_f_glsl_size);
        programs[TEXTURED_PROGRAM] =
            Program(_shader_texture_v_glsl, _shader_texture_v_glsl_size,
                    _shader_texture_f_glsl, _shader_texture_f_glsl_size);
        programs[FONT_PROGRAM] =
            Program(_shader_font_v_glsl, _shader_font_v_glsl_size,
                    _shader_font_f_glsl, _shader_font_f_glsl_size);
        programs[FONT_PROGRAM_DF] =
            Program(_shader_font_v_glsl, _shader_font_v_glsl_size,
                    _shader_fontdf_f_glsl, _shader_fontdf_f_glsl_size);
    }
...
void Program::setUniform(const std::string& name, float f0, float f1,
                         float f2) const
{
    auto h = getUniformLocation(name);
...
void Program::vertexAttribPointer(const std::string& name, GLint size,
                                  GLenum type, GLboolean normalized,
                                  GLsizei stride, const GLvoid* ptr) const
{
...
void Program::vertexAttribPointer(const std::string& name, GLint size,
                                  GLenum type, GLboolean normalized,
                                  GLsizei stride, GLint offset) const
{
    GLuint h = getAttribLocation(name);
    glVertexAttribPointer(h, size, type, normalized, stride,
