...
TileSet::TileSet(shared_ptr<ImagePacker> packer)
    : packer(packer), texture(packer->size().w, packer->size().h)
{
...
int TileSet::add(const bitmap& bm)
{
    ...
    glBindTexture(GL_TEXTURE_2D, texture.id());
    auto flipped = bm.flip();
    glTexSubImage2D(GL_TEXTURE_2D, 0, xpos, texture.height() - tileh - ypos,
                    tilew, tileh, GL_RGBA, GL_UNSIGNED_BYTE, flipped.data());
...
void TileSet::renderTile(int tileno, RenderTarget& target, float x, float y,
                         double s)
{
...
void save_data(utils::path const& p, const TileSet& data)
{

    const auto& bm = data.texture.get_pixels();
    image::save_png(bm, p.string());
    File mf{p.string() + ".tiles"};
...
template <> shared_ptr<TileSet> load_data(utils::path const& p)
{
    auto ts = make_shared<TileSet>();
    auto bm = image::load_image(p.string());
    ts->texture = Texture(bm);
    File mf{p.string() + ".tiles"};
    for (const auto& t : mf.lines()) {
        auto p = split(t, " ");
        ts->tiles.emplace_back(stof(p[0]), stof(p[1]), stof(p[2]), stof(p[3]),
                               stol(p[4]), stol(p[5]));
    }
    mf.close();
    return ts;
...
// TileLayer::TileLayer(uint32_t pw, uint32_t ph, uint32_t tw, uint32_t th,
// const TileSet &ts) : 	scrollx(0), scrolly(0), scale(1.0), tileset(ts),
// pixel_width(pw), pixel_height(ph), tile_width(tw), tile_height(th),
// tileSource(nullptr) {}

TileLayer::TileLayer(shared_ptr<TileSet> ts, shared_ptr<TileSource> source,
                     uint32_t tw, uint32_t th)
    : scrollx(0), scrolly(0), scale(1.0), tileset(ts), tile_width(tw),
      tile_height(th), tileSource(source)
{
    if (ts) {
        if (tile_width == 0)
...
void TileLayer::render(RenderTarget& target)
{
    ...
    if (multiBuf[0] == -1) {
        ...
        glBufferData(GL_ARRAY_BUFFER, coords.size() * 4 * 2, &coords[0],
                     GL_DYNAMIC_DRAW);
    } else
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, multiBuf[1]);
    ...
    // program.setUniform("vScreenScale", 2.0 / target.width(), 2.0 /
    // target.height(), 0, 1); program.setUniform("vScale", 1.0, 1.0, 0, 1);
    ...
    // glScissor(x0, target.height()-pixel_height-y0, pixel_width,
    // pixel_height);
...
shared_ptr<Sprite> SpriteLayer::addSprite(int tileno, float x, float y,
                                          float scale)
{
    purgeSprites();
...
shared_ptr<Sprite> SpriteLayer::addSprite(vector<int> frames, float x, float y,
                                          float scale)
{
    purgeSprites();
...
void SpriteLayer::render(RenderTarget& target, int x, int y)
{
    ...
    while (i != sprites.end()) {
        ...
        auto s = i->lock();
        if (s) {
            auto& t = s->tileno < 0 ? tileset->tiles[s->frames[s->frame]]
                                    : tileset->tiles[s->tileno];
            vector<float> uvs = {t.s0, t.t0, t.s1, t.t0,
                                 t.s0, t.t1, t.s1, t.t1};
            target.draw(tileset->get_texture(), (int)s->x + x - scrollx,
                        (int)s->y + y - scrolly, t.w * s->scale, t.h * s->scale,
                        &uvs[0]);
            ++i;
        } else
            i = sprites.erase(i);
